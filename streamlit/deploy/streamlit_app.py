...
if page == "Margins":
    ...
    margin_df = session.sql(f"""
        SELECT PRODUCT_NAME, BRAND, CATEGORY, SUB_CATEGORY,
               GROSS_MARGIN_PCT, TOTAL_REVENUE, BASE_PRICE, COST,
    ...
    if not margin_df.empty:
        margin_df["TOTAL_REVENUE"] = pd.to_numeric(margin_df["TOTAL_REVENUE"], errors="coerce").fillna(0)
        margin_df["GROSS_MARGIN_PCT"] = pd.to_numeric(margin_df["GROSS_MARGIN_PCT"], errors="coerce").fillna(0)
        positive = margin_df[margin_df["TOTAL_REVENUE"] > 0]
    ...
    promo_df = session.sql("""
        SELECT PROMO_NAME, PROMO_TYPE, DISCOUNT_PCT, PRODUCTS_IN_PROMO,
               PROMO_UNITS_SOLD, PROMO_REVENUE, REVENUE_PER_PRODUCT,
               START_DATE, END_DATE
    ...
    if not promo_df.empty:
        promo_df["PROMO_REVENUE"] = pd.to_numeric(promo_df["PROMO_REVENUE"], errors="coerce")
        promo_df["DISCOUNT_PCT"] = pd.to_numeric(promo_df["DISCOUNT_PCT"], errors="coerce")
    ...
    if not comp_df.empty:
        comp_df["PRICE_INDEX"] = pd.to_numeric(comp_df["PRICE_INDEX"], errors="coerce")
    ...
    abc_df = session.sql("""
        SELECT STORE_NAME, REGION, ABC_CLASS, COUNT(*) AS PRODUCT_COUNT,
    ...
    if not abc_df.empty:
        abc_df["REVENUE_30D"] = pd.to_numeric(abc_df["REVENUE_30D"], errors="coerce")
        ...
        c1.metric("A (Star)", f"{a_count:,}")
        c2.metric("B (Core)", f"{b_count:,}")
        c3.metric("C (Tail)", f"{c_count:,}")
        c4.metric("D (Dead)", f"{d_count:,}")

        st.divider()
        st.subheader("Revenue Forecast by Category")
        forecast_df = session.sql("""
            SELECT SERIES AS CATEGORY, TS AS FORECAST_DATE, ROUND(FORECAST, 0) AS FORECAST_REVENUE
